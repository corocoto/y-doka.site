...
Во всех примерах далее будет рассматриваться направление письма, характерное для кирилицы и латиницы (слова — слева направо, блоки — сверху вниз). Но все те же объяснения можно применять и для другого направления письма.
...
Строчные элементы располагаются друг за другом слева направо, как слова в предложении. Ширина и высота строчного элемента равна ширине и высоте содержимого. В отличие от блочного элемента, мы не можем управлять шириной и высотой строчного через CSS. Несколько строчных элементов будут стремиться находиться на одной строке, пока их все можно уместить в ширину родителя. Если ширины родителя не хватает, то лишний текст строчного элемента переносится на следующую строку.
...
- задать родителю вертикальное поле [`padding-top`](/css/doka/padding) или [`padding-bottom`](/css/doka/padding) в зависимости от того, с какой стороны мы хотим предотвратить выпадение;
- задать родителю верхнюю или нижнюю [границу](/css/doka/border) по такой же логике. Граница может быть прозрачной, главное, чтобы она была :);
...
Ранее мы выяснили, что все элементы по умолчанию находятся в нормальном потоке. Однако, существуют несколько CSS-свойств, изменение которых приводит к тому, что элемент перестаёт взаимодействовать с остальными блоками в потоке. Говорят, что он «вышел из потока».
